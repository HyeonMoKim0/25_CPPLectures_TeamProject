...
public:
	...
	void exchange() { exchangeMoney = foreignCurrency() * exchangeRate; }
...
int main() {
	...
	//ExchangeMoney<USDMoney> exchangedMoney(myDellar); // 어댑터 객체 생성 및 환전 수행
	KoreanMoney* myWon = new ExchangeMoney<USDMoney>(myDollar); // 어댑터로 환전된 원화 객체 생성

	//cout << exchangedMoney.getMoney() << "원 환전 완료!" << endl;
